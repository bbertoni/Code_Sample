...
axs = axs.ravel()
plt.suptitle("Bullock & Johnston data "+str(halo)+", footprint, |vLOS| < 600 km/s, |b| > 30 deg, random noise in dist and vLOS")
for i in range(len(rvals)-1):
	r = list()
	psi1 = list()
	counter = list()
	infile = open("/home/bridget/BullockJohnston/footprint_mangle/"+str(halo)+"/JBout7psi1_polygon_"+
                       str(rvals[i])+"_"+str(rvals[i+1])+"_10.0.txt",'r')
...
fig.savefig("JB_mangle_"+str(halo)+".png")
